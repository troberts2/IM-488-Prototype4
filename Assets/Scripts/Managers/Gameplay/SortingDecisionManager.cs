    ...
    private bool ObjectShouldBeRecycled()
    {
        if (GameObject.FindGameObjectWithTag("Conveyor").GetComponent<Conveyor>().currentTrash != null)
        {
            ...
            foreach (TrashObj.material material in materialTypes)
            {
                ...
                if(material == TrashObj.material.Uranium && itemEffectsList.Count > 0 && itemEffectsList[0] != "wash") return true;
                else if(material == TrashObj.material.Uranium  && itemEffectsList.Count == 0 || itemEffectsList.Count > 1) return true;
